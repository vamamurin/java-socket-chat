...
public class LoginTUI {
    ...
    private StringBuilder friendBuffer = new StringBuilder();
    private int currentField = 0; // 0=username, 1=password, 2=friend
    private String errorMessage = "";
    private boolean isLoggedIn = false;
    private boolean running = true;

    // Colors (matching ChatTUI)
    ...
    public LoginTUI() throws IOException {
    ...
    public void run() throws IOException {
        ...
        if (isLoggedIn) {
            screen.stopScreen();
            // Launch chat TUI
            new ChatTUI(usernameBuffer.toString(), friendBuffer.toString()).run();
        } else {
            screen.stopScreen();
        }
    }
    ...
    private void attemptLogin() {
        ...
        String friend = friendBuffer.toString().trim();

        if (user.isEmpty() || pass.isEmpty() || friend.isEmpty()) {
    ...
    private void drawLoginInterface() {
        ...
        // Draw main login box
        drawBox(boxX, boxY, boxWidth, boxHeight, "LOGIN", ACCENT_CYAN);
        ...
        drawFormField(boxX + 2, formStartY + 6, boxWidth - 4, "Chat with", friendBuffer.toString(), currentField == 2);
        ...
        String[] instructions = {
            "TAB/↑↓: Navigate | ENTER: Submit | ESC: Exit"
        };
    ...
    private void drawStatusBar(int width) {
        ...
        String status = " 🔐 SECURE LOGIN | AES-256 ENCRYPTED ";
